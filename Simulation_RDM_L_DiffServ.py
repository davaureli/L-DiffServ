...
print()
print("Example classic DSCP")
print(file_classic_DSCP[0])
print()
print("Example new DSCP")
...
IDEA: For statistically recreate traffic but forn now not used
...
for mark in poss_mark :
    ...
    for cap in [300000000, 400000000, 500000000, 600000000, 700000000, 800000000, 900000000, 1000000000]:
        ...
        #tot = 10000000 * 0.250 / sqrt(# of sessions)
        ...
        #Choose the data structure according to the marking analysis
        if mark == "old":
        ...
        while time_sequence:
            ...
            #We extract the first element of the tree the Root
            ...
            if event == 0: #new packet
                ...
                #Check on queue dimension
                if sum(queues[d_type]) + size > queue_max_length[d_type]: #full queue
                    
                    percentage_full_queue = [0]*number
                    selected_queue = -1

                    
                    for type_que in range(number):

                        percentage_full_queue[type_que] = sum(queues[type_que])/queue_max_length[type_que]

                        if percentage_full_queue[type_que] < 0.8 and sum(queues[type_que]) + size <= queue_max_length[type_que]:
                            selected_queue = type_que
                            break
                    
                    
                    if selected_queue != -1:
                        queues[selected_queue].append(size)

                        if len(queues[selected_queue]) == 1: #empty queue
                            #print("First Packet in his queue type")
                            
                            #add new event to time_sequence = packet elaborated
                            time_to_complete = size/router_speed[selected_queue]
                            #updating time
                            time_completed = current_time + time_to_complete
                            
                            hq.heappush(time_sequence,(time_completed,(1,type_que,size,block,real_DSCP))) #size here is useless
        ...
        for pkt in (discarded_type):
            ...
            if pkt[1][3] in diz_hash_discarded:
                #Update the count of packet discarded according to the specific session
                diz_hash_discarded[pkt[1][3]] += 1
                
        #Second Queue
        for pkt in (discarded_last_type):
            #Estrazione dell'hash relativa all'identificazione del flusso
            if pkt[3] in diz_hash_discarded:
                #Update the count of packet discarded according to the session
                diz_hash_discarded[pkt[3]] += 1
...
#print(discarded_type[0])
#print(discarded_last_type[0])



#Save the dataframe for the plot Confidence Interval
with open('./RDM/RDM_DSCP_Current_0528_share08.pkl', 'wb') as f:
    pickle.dump(flussi_colpiti_current_DSCP, f)
with open('./RDM/RDM_DSCP_Proposed_0528_share08.pkl', 'wb') as f:
    pickle.dump(flussi_colpiti_proposal_DSCP, f)

#print(flussi_colpiti_current_DSCP)
#print(flussi_colpiti_proposal_DSCP)
print("Saved")