...
def extract_Info_pckt(file_name, lista_packet_ICMP):
    ...
    #VM (lasciare sempre così perché funziona sia per VM che Locale)
    ...
    #                dst_port = packet.tcp.dstport
                ...
                #Protocol as IP and ICMP e Ws.Short will have Port values as -1
                valori.append(-1)
                valori.append(-1)
    ...
    #                dst_port = -1
            ...
            if "ICMP" in packet:
                ...
                #Sniffing Time
                time_1 = float(packet.sniff_timestamp)
                if time <time_1:
                    time = time_1
...
#            hash_id = hash(source + destination + str(src_port) + str(dst_port) + str(protocol)) 
#            
#            if hash_id not in dictionary_excludeHash:
#                
...
#                print("NON INSERITO")
    ...
    with open('../shareClusterJSAC/' + title + '.pkl', 'wb') as f:
        pickle.dump(tot_dat, f)
        
    print("Saved into shareCluster")
    
    print()
    print("Here we have analyzed this number of pkts: " + str(i))
    print()
    ...
    print("Final Time according to the Timestamp:")
    print()
    print(time)
    print()
...
def Statistics(dictionary, total_packet):
    ...
    labels =  ["best effort","Immediate","Flash voice","Internetwork control","Critical voice RTP",
               "Priority","Network Control","Not Known"]

    #Stiamo creando le liste dove mettiamo le percentuali per classe
    sizes = [ round(percentage[elem]*100/sum(percentage.values()),4)  for elem in labels if elem in percentage]
    ...
    legend = ["Best Effort","Immediate","Flash Voice","Internetwork Control","Critical Voice RTP",
              "Priority","Network Control","Not Known"]
